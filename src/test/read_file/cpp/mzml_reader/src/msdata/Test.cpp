//
// $Id: SpectrumList_mzML_Test.cpp 6478 2014-07-08 20:01:38Z chambm $
//
//
...
#include "pwiz/data/msdata/Serializer_mzML.hpp" // depends on Serializer_mzML::write() only
...
void test(bool indexed)
{
    ...
    // if (os_) *os_ << sl->find("sample=1 period=1 cycle=23 experiment=6") << endl;
    ...
    //     if (os_) *os_ << pairs[i].mz << "\t" << pairs[i].intensity << endl;
    // }
    // if (os_) *os_ << "scan=19 : " << s->scanList.scans[0].spectrumID << endl;
